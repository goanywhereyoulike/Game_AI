...
{
	mNodes.clear();
	mNodes.resize(static_cast<size_t>(columns) * rows);
	mColumns = columns;
	mRows = rows;
	...
	for (int r = 0; r < rows; ++r)
	{
		for (int c = 0; c < columns; ++c)
		{
			...
			node->neighbors[1] = GetNode(c, r + 1);
			node->neighbors[2] = GetNode(c - 1, r);
			node->neighbors[3] = GetNode(c + 1, r);
			node->neighbors[4] = GetNode(c - 1, r - 1);
			node->neighbors[5] = GetNode(c + 1, r - 1);
			node->neighbors[6] = GetNode(c - 1, r + 1);
