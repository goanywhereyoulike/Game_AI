...
void MushroomWorld::Load()
{
	...
	mMushroomActive.resize(MushroomCount, true);
...
void MushroomWorld::Update(float deltaTime)
{
}
...
void MushroomWorld::Render()
{
		...
		{
			if (!mMushroomCollected[i]&& mMushroomActive[i])
...
void MushroomWorld::KillMushroom(size_t index)
{
	mMushroomActive[index] = false;
}
...
{
	size_t left = 0;
	for (auto alive :mMushroomActive)
	{
		//if (alive)
			left++;
	}
...
bool MushroomWorld::IsMushroomAlive(size_t index) const
{
	return mMushroomActive[index];
}
